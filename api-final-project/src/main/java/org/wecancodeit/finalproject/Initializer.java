...
public class Initializer implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		// Users
		User userOne = userRepo.save(new User("binieth",
				"https://raw.githubusercontent.com/madlaw10/final-project/master/react-final-project/src/media/avatar-binieth.jpg"));
		User userTwo = userRepo.save(new User("dexter",
				"https://raw.githubusercontent.com/madlaw10/final-project/master/react-final-project/src/media/avatar-jbunny.png"));
		User userThree = userRepo.save(new User("madlaw10",
				"https://raw.githubusercontent.com/madlaw10/final-project/master/react-final-project/src/media/avatar-madlaw10.png"));
		User userFour = userRepo.save(new User("nate-fritz",
				"https://raw.githubusercontent.com/madlaw10/final-project/master/react-final-project/src/media/avatar-nate-fritz.png"));
		User userFive = userRepo.save(new User("eekramm",
				"https://raw.githubusercontent.com/madlaw10/final-project/master/react-final-project/src/media/avatar-eekramm.jpg"));

		// PreCleanUps
		PreCleanUp preCleanUpZero = preCleanUpRepo.save(new PreCleanUp(LocalDate.now(), "Portal Park",
				"We are all gathering at Goodale Park again to pick up any trash that has accumulated since our last CleanUp!  Please join us if you're available!",
				userOne));
		PreCleanUp preCleanUpOne = preCleanUpRepo.save(new PreCleanUp(LocalDate.now(), "Glen Echo Park",
				"Hey guys!  Park of Roses is beautiful, but we've noticed some trash blowing in from recent storms.  Let's get together and CleanUp!",
				userTwo));
		PreCleanUp preCleanUpTwo = preCleanUpRepo.save(new PreCleanUp(LocalDate.now(), "Clinton-Como Park",
				"We've got another CleanUp going! Head over to help out if you can.", userThree));
		PreCleanUp preCleanUpThree = preCleanUpRepo
				.save(new PreCleanUp(LocalDate.now(), "Glen Echo Park", "Let's do this!", userFour));
		PreCleanUp preCleanUpFour = preCleanUpRepo.save(new PreCleanUp(LocalDate.now(), "Tuttle Community Recreation Center",
				"Starts at 11am.  We'll meet in the first parking lot after the welcome center.", userFive));

		// PostCleanUps
		PostCleanUp cleanUpOne = postCleanUpRepo.save(new PostCleanUp(
				...
				"Riverside Park", "Riverside clean up when well with great friends!", userOne));
		PostCleanUp cleanUpTwo = postCleanUpRepo.save(new PostCleanUp(
				...
				"Miami Beach", "Beaches look better without trash.", userTwo));
		PostCleanUp cleanUpThree = postCleanUpRepo.save(new PostCleanUp(
				...
				"Lake Erie", "Cleaned up the beach this weekend.", userThree));
		PostCleanUp cleanUpFour = postCleanUpRepo.save(new PostCleanUp(
				"https://static.boredpanda.com/blog/wp-content/uploads/2019/03/xji67arzwbl21-5c86559301cbe__700.jpg",
				"Manila Bay", "CleanUp at Manila Bay.", userFour));
		PostCleanUp cleanUpFive = postCleanUpRepo.save(new PostCleanUp(
				...
				"Potomac River", "Cleaned up DC a bit.", userFive));
